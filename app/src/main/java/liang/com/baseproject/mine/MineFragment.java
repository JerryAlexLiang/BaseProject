 ...
 */
public class MineFragment extends MVPBaseFragment implements CustomPopupWindow.ViewInterface, View.OnClickListener {
    ...
    private void initUserInfo() {
        if (UserLoginUtils.getInstance().isLogin()) {
            ...
            if (!TextUtils.isEmpty(localUserIcon) && !TextUtils.isEmpty(localBg)) {
                ...
                Glide.with(mActivity).asBitmap().load(R.drawable.icon_navigation_bg).into(ivBlur);
            }
    ...
    @OnLongClick({R.id.civ_user_icon})
...
//                AnimationUtils.pageHideScaleAnimator(rlPageContainer);
    ...
    public void getChildView(View view, int layoutResId, int flag) {
        ...
        TextView mRecordVideo = view.findViewById(R.id.btn_record_video);
        TextView mLookImage = view.findViewById(R.id.btn_look_image);
        TextView mDismissWindow = view.findViewById(R.id.btn_dismiss_window);
        ...
        if (flag == 0) {
            ...
            mLookImage.setVisibility(View.VISIBLE);
        } else if (flag == 1) {
            viewLineFour.setVisibility(View.GONE);
            mLookImage.setVisibility(View.GONE);
        }

        //选项Button点击监听事件
        mSelectPhotoBtn.setOnClickListener(this);
        mTakePhotoBtn.setOnClickListener(this);
        mLookImage.setOnClickListener(this);
        mDismissWindow.setOnClickListener(this);
