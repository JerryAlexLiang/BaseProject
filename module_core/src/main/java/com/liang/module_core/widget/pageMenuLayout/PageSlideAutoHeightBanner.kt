...
import com.liang.module_core.widget.AutoHeightViewPager
 ...
 * 作者: 杨亮
 */
class PageSlideAutoHeightBanner<T> @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : RelativeLayout(context, attrs, defStyleAttr) {
    private var mSlideMenuBannerViewPager: AutoHeightViewPager? = null
    ...
    private fun initView(context: Context, attrs: AttributeSet?) {
        mSlideMenuBannerViewPager = AutoHeightViewPager(context)
    ...
    val pageCount: Int
        get() = if (mSlideMenuBannerViewPager != null && mSlideMenuBannerViewPager?.adapter != null) {
            mSlideMenuBannerViewPager?.adapter?.count ?: 0
        } else {
            0
        }
    ...
    fun currentViewPager(): AutoHeightViewPager? {
        return mSlideMenuBannerViewPager
    }
    ...
    fun setPageData(rowCount: Int, spanCount: Int, data: List<T>, creatorSlide: PageSlideMenuViewHolderCreator) {
        ...
        for (index in 0 until pageCount) {
            ...
            mSlideMenuBannerViewPager?.setViewForPosition(recyclerView, index)
            mSlideMenuBannerViewPager?.isScrollable = true
        }
