 ...
 * 作者: 杨亮
 */
class EyeRepository private constructor(private val eyeNetwork: EyeNetwork) {

    fun fetchDiscoveryData(url: String) = fire(Dispatchers.IO) {
        val fetchDiscoveryData = eyeNetwork.fetchDiscoveryData(url)
        Result.success(fetchDiscoveryData)
    }
    ...
    companion object {
        ...
        private var repository: EyeRepository? = null

        fun getInstance(network: EyeNetwork): EyeRepository {
            if (repository == null) {
                synchronized(EyeRepository::class.java) {
                    if (repository == null) {
                        repository = EyeRepository(network)
                    }
                }
            }
            return repository!!
